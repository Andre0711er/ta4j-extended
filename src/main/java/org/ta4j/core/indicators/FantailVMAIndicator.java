...
public class FantailVMAIndicator extends AbstractIndicator<Num> {
    ...
    private static class VarMAIndicator extends CachedIndicator<Num> {
        ...
        protected Num calculate(int index) {
            ...
            Num constVal = diff.getValue(index).isPositive() ? adx.getValue(index)
                    .minus(adxLow.getValue(index))
                    .dividedBy(diff.getValue(index)) : getBarSeries().numFactory().zero();
            return getValue(index - 1).multipliedBy(getBarSeries().numFactory().numOf(2).minus(constVal)).plus(
    ...
    private static class STRIndicator extends CachedIndicator<Num> {
        ...
        protected Num calculate(int index) {
            ...
            return getValue(index - 1).multipliedBy(weighting).plus(tr).dividedBy(
                    weighting.plus(getBarSeries().numFactory().one()));
        }
